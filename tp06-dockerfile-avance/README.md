# TP6 — Dockerfile avancé : multi-stage build

> Durée estimée : 50 min · Port utilisé : `8088`
> Prérequis : avoir fait le **TP4** et le **TP5** (écrire un `Dockerfile`, des fichiers statiques jusqu'à une appli complète).
> Ce TP approfondit le chapitre **« Créer ses propres images »** du jour 1 (couches, cache, multi-stage, non-root).

## 🎬 Le contexte

Au TP4, vous avez emballé des fichiers statiques dans une image nginx : aucun **build** n'était nécessaire. Mais la plupart des applis doivent être **compilées** (Go, Java, TypeScript…) avant d'être exécutées.
...
```bash
cd solution
docker build -f Dockerfile.naive -t tp06-naive:1.0 .
docker images tp06-naive:1.0
```
...
```bash
docker build --build-arg VERSION=tp06-1.0 -t tp06-api:1.0 .
docker run -d --name api -p 8088:8080 tp06-api:1.0
curl -s http://localhost:8088/         | grep "Telemach"
curl -s http://localhost:8088/version  # → tp06-1.0
docker images tp06-api:1.0             # comparez avec tp06-naive !
```

> ❓ **Question** : comparez la taille de `tp06-api:1.0` (multi-stage) avec `tp06-naive:1.0`. Quel facteur de réduction obtenez-vous ?
...
1. **Construire une seule étape.** `docker build --target build -t tp06-builder .` : seule l'étape `build` est construite. À quoi cela sert-il en CI (lancer les tests dans l'étape de build sans produire l'image finale) ? → doc : https://docs.docker.com/build/building/multi-stage/#stop-at-a-specific-build-stage
