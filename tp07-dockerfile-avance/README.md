# TP7 — Dockerfile avancé : multi-stage build

> Durée estimée : 50 min · Port utilisé : `8088`
> Prérequis : avoir fait le **TP5** et le **TP6** (écrire un `Dockerfile`, des fichiers statiques jusqu'à une appli complète).
> Ce TP approfondit le chapitre **« Créer ses propres images »** du jour 1 (couches, cache, multi-stage, non-root).

## 🎬 Le contexte

Au TP5, vous avez emballé des fichiers statiques dans une image nginx : aucun **build** n'était nécessaire. Mais la plupart des applis doivent être **compilées** (Go, Java, TypeScript…) avant d'être exécutées.
