# TP5 — Dockerfile : toutes les instructions courantes

> Durée estimée : 50 min · Port utilisé : `8085`
> Prérequis : avoir fait le **TP4** (écrire un premier `Dockerfile` pour des fichiers statiques).
> Ce TP approfondit le chapitre **« Créer ses propres images »** du jour 1.

## 🎬 Le contexte

Au TP4, votre `Dockerfile` faisait deux choses : `FROM` + `COPY`. C'est suffisant pour une page statique, mais une **vraie appli** doit installer des dépendances, recevoir une configuration, déclarer son port, tourner sans privilèges et signaler sa santé.
...
```bash
docker build --build-arg APP_VERSION=1.4.2 -t tp05-portail:1.4.2 .
```

Lancez le conteneur (on publie le port 3000 du conteneur sur 8085 de l'hôte) :

```bash
docker run -d --name tp05-portail -p 8085:3000 tp05-portail:1.4.2
...
docker exec tp05-portail id                 # uid=1000(node) → PAS root ✅
docker inspect -f '{{json .Config.Healthcheck}}' tp05-portail:1.4.2
```
...
```bash
docker run --rm tp05-portail:1.4.2 --version   # exécute « node --version » (CMD remplacé)
```
...
```bash
docker rm -f tp05-portail && docker rmi tp05-portail:1.4.2
```
